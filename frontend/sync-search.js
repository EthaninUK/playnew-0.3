...
async function syncProviders() {
  ...
  try {
    const response = await fetch(`${DIRECTUS_URL}/items/service_providers?limit=-1&fields=id,name,slug,category,description,rating`);
...
async function syncNews() {
  ...
  try {
    const response = await fetch(`${DIRECTUS_URL}/items/news?limit=1000&fields=id,title,slug,summary,category,published_at`);
